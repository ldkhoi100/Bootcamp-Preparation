...
function IsValidRowToTotal(row, number){
    ...
    return number<=row && number>0 && !isNaN(number) && Number.isInteger(number);
}
...
function TotalMultipleOf5(row, col, matrix){
    let total = 0;
    for(i=0; i<row;i++){
        for(j=0;j<col;j++){
            if(matrix[i][j] %5 == 0){
...
// Tìm và trả về giá trị lớn nhất và từng vị trí của nó trong ma trận.
...
// Dòng có tổng giá trị lớn nhất của ma trận
...
function Main(){
    ...
    if(validRow && validCol && numberRow){
        ...
        document.getElementById("TotalMultipleOf5").innerHTML = TotalMultipleOf5(row, col, matrix);
